::  The largest SCC of the identity-level call graph of the scow poke, on the
...
::
:-  %noun
:-  (crip "self-calling {(scow %ud ~(wyt in selfs))} self-single {(scow %ud self-single)} ids {(scow %ud ~(wyt by g))} sccs {(scow %ud (lent sccs))} big {<(scag 10 sizes)>} entry {(area entry)} of {(scow %ud ~(wyt in big))} visited {(scow %ud ~(wyt in seen.res))}")
(flop out.res)
