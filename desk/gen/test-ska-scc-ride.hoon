::  SCC structure of the identity-level and bell-level call graphs after the
::  ride poke
::
/+  *nock-compilation
/+  hoot-zpdt
...
=/  bg=(jug bell bell)  -:(simple-bell-graph-and-reversed g)
=/  bell-sizes=(list @)
  (sort (turn (tarjan bg) |=(s=(set bell) ~(wyt in s))) gth)
:-  %noun
:*  boot-ids+n-boot
    ids+~(wyt by g)
    fols+fols
    ...
    bell-sccs+(lent bell-sizes)
    big-bell-sccs+(scag 12 bell-sizes)
==
